...
export async function generateAlternativeTitles(
  ...
  const prompt = `
    ...
    - For each title, ensure the most critical keywords are front-loaded. If no priority keyword is provided, start each title with a different high-volume keyword from the provided list.
...
export async function regenerateKeyword(
  ...
  const prompt = `
    ...
    - Must be a maximum of 20 characters, including spaces.
